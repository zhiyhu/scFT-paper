...
CalculateDEMatrix <- function(object,
  ...
  for (k in 1:length(m_de_list)) {
    ...
    if (sampling == T) { # sampling 80% of the samples
      ...
      idx1 <- which(colData(object)$initial.cluster %in% c(i))
      idx2 <- which(colData(object)$initial.cluster %in% c(j))
      lgt <- min(floor(length(idx1) * 0.8), floor(length(idx2) * 0.8)) #size of each group
      res.n <- c()

      for (ns in 1:n.sampling) { # do sampling DE analysis for n.sampling times
        idx1 <- sample(x = idx1, size = lgt )
        idx2 <- sample(x = idx2, size = lgt )
        index <- c(idx1, idx2)
